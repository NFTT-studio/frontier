...
use precompile_utils::{Bytes, EvmDataReader, EvmResult, FunctionModifier, Gasometer, RuntimeHelper};
	...
	T::Call: From<frame_system::Call<T>>,
{
	fn execute(
		...
		gasometer.check_function_modifier(
			...
			is_static,
			match selector {
				Action::RemarkWithEvent => FunctionModifier::NonPayable,
		...
		match selector {
			...
			Action::RemarkWithEvent => Self::remark_with_event(&mut input, &mut gasometer, context),
		}
	}
}
